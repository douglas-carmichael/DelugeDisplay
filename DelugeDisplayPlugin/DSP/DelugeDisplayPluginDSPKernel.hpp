//
//  DelugeDisplayPluginDSPKernel.hpp
//  DelugeDisplayPlugin
//
...
#import "DelugeDisplayPluginParameterAddresses.h"

constexpr uint16_t kMaxVelocity = std::numeric_limits<std::uint16_t>::max();

/*
 DelugeDisplayPluginDSPKernel
 As a non-ObjC class, this is safe to use from render thread.
 */
class DelugeDisplayPluginDSPKernel {
public:
    ...
    // Add a case for each parameter in DelugeDisplayPluginParameterAddresses.h
    void setParameter(AUParameterAddress address, AUValue value) {
        switch (address) {
            case DelugeDisplayPluginParameterAddress::midiNoteNumber:
                mNextNoteToSend = (uint8_t)value;
                break;
            case DelugeDisplayPluginParameterAddress::sendNote:
    ...
    void handleMIDIEventList(AUEventSampleTime now, AUMIDIEventList const* midiEvent) {
        /*
         ...
         auto thisObject = static_cast<DelugeDisplayPluginDSPKernel *>(context);
